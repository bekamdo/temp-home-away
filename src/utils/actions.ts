...
export const fetchFavoritesId= async({propertyId}:{propertyId:string}) => {
  const user = await getAuthUser();
...
export const toggleFavouriteAction = async(prevState:{
  ...
  return {message:"toggle favourites"}
}
...
export const createReviewAction = async (prevState: any, formData: FormData) => {
  ...
  try{
    ...
    return { message: 'Review Submited Succesfully' };
  }catch(error){
   return renderError(error)
  }
...
export const createBookingAction = async(prevState:{propertyId:string,checkIn:Date,checkOut:Date}) => {
  ...
  if(!property){
    return {message:"Property is not found"}
  }
...
export const updatePropertyImageAction = async (prevState:any,formData:FormData):Promise<{message:string}> => 
  {
    ...
    try{
      ...
      revalidatePath(`/rentals/${propertyId}/edit`);
      return { message: 'update property image succefully' };
    }catch(error){
      return renderError(error)
    }
